...
export const wildriftChampions = atom([]);
export const wildriftItems = atom([]);
export const wildriftRunes = atom([]);
export const WildriftConstantes = deepMap({})
...
export const urlPositionLaners = (laner) =>{
  const url = "/wildrift/lanes/";
  const fullUrl = {
    Top: `${url}top.webp`,
    Jungle: `${url}jungle.webp`,
    Mid: `${url}mid.webp`,
    Bot: `${url}bottom.webp`,
    Support: `${url}support.webp`
  }
...
export const fetchMeta = task(async () => {
  ...
  wildriftChampions.set(champions);
...
export const fetchConstantesWildrift = async ()=>{
  task(
    async ()=>{
      try{
        const token = import.meta.env.PUBLIC_TOKEN_META;
        console.log("Fetching constantes Wildrift data with token:");
