              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!user || !user.email) {
                        const appliedCoupon = couponStates[plan.id]?.status === 'success' ? couponStates[plan.id].code : null;
                        const redirectUrl = appliedCoupon
                          ? `/tft/master-plan?coupon=${appliedCoupon}`
                          : `/tft/master-plan`;
                    ...
                    className={`${style.button} ${isFeatured ? style.buttonPrimary : style.buttonSecondary
                      }`}
                  >
                    {isLoading ? (
                      <span className={style.loadingSpinner}></span>
                    ) : (
                      `Activar Master Plan ${couponStates[plan.id]?.status === 'success' ? `(Dcto. Aplicado)` : ''}`
                    )}
                  </button>
                </div>
              );
